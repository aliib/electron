...
const {URLRequest, WebSocket} = net
...
WebSocket.prototype.send = function(data, fin) {
    if (fin !== false) {
      // fin is either true or false, nothing else. Default to true.
      fin = true
    }

    let dataIsString = typeof data === 'string'
    let dataIsBuffer = data instanceof Buffer
    if (!dataIsString && !dataIsString) {
        throw new TypeError('First argument must be a string or Buffer.')
    }

    let opCode = WebSocketOpCode.kOpCodeBinary
    if (dataIsString) {
        // We convert all strings into binary buffers.
        opCode = WebSocketOpCode.kOpCodeText
        data = Buffer.from(data)
    }

    this._send(data, opCode, fin)
}
...
WebSocket.prototype.ping = function(data) {
    ...
    if (!(data instanceof Buffer)) {
        throw new TypeError('data must be an instance of Buffer')
    }

    this._send(data, WebSocketOpCode.kOpCodePing, true);
}
...
WebSocket.prototype.pong = function(data) {
    ...
    this._send(data, WebSocketOpCode.kOpCodePong, true);
}

WebSocket.prototype.close = function(code, reason) {
    if (typeof code === 'string') {
      // Use defaut status code. The user provided only the reason.
      code = 1000
      reason = code
        ...
        throw new TypeError('First argument must be a status code or a reason')
    }
...
Object.defineProperty(WebSocket.prototype, 'binaryType', {
    ...
    set: function(value) {
        if (value !== 'nodebuffer' && value !== 'arraybuffer') {
            throw new Error("binaryType value must be either a 'nodebuffer' or 'arraybuffer'")
        }
